...
//Calcul de la repartition de la chaleur pour une matrice 1D
...
//Calcul de la repartition de la chaleur pour une matrice 2D
	...
	//diffusion
		...
		for(i=0;i<nbLigne;i++) {
			for(j=0;j<tailleLigne;j++) {
				...
				if(j == 0){
					tmp = 4*tab[i*tailleLigne] + 2 * tab[i*tailleLigne+1];
				}else if(j == tailleLigne-1){
					tmp = 4*tab[i*tailleLigne+j] + 2 * tab[i*tailleLigne+j-1];
				}else{
		...
		//cellule haut gauche
		tmp_tab[0] = 4*tab[0]+ 2 * tab[1];
		//cellule haut droite
		tmp_tab[tailleLigne-1] = 2 * tab[tailleLigne-2]+ 4 * tab[tailleLigne-1];
		//cellule bas gauche
		tmp_tab[tailleLigne * (nbLigne -1)] = 4*tab[tailleLigne * (nbLigne -1)]+ 2 * tab[tailleLigne * (nbLigne -1) + 1];
		//cellle bas droite
		tmp_tab[tailleLigne * (nbLigne)-1] = 4*tab[tailleLigne * (nbLigne)-1]+ 2 * tab[tailleLigne * (nbLigne)-2];
		...
		for(j=0;j<tailleLigne;j++) {
			for(i=0;i<nbLigne;i++) {
				//Somme sur les valeurs verticale
				if(i == 0){
					tmp = 4*tab[j] + 2 * tab[j+tailleLigne];
				}else if(i == nbLigne -1){
					tmp = 4*tab[(nbLigne-1)*tailleLigne+j] + 2*tab[(nbLigne-2)*tailleLigne+j];
				}else{
		...
		//cellule haut gauche
		tmp_tab[0] = 4*tab[0]+ 2*tab[tailleLigne];
		//cellule haut droite
		tmp_tab[tailleLigne-1] = 2*tab[tailleLigne-1 + tailleLigne]+ 4*tab[tailleLigne-1];
		//cellule bas gauche
		tmp_tab[tailleLigne * (nbLigne -1)] = 4*tab[tailleLigne * (nbLigne -1)]+ 2*tab[tailleLigne * (nbLigne -1) - tailleLigne];
		//cellle bas droite
		tmp_tab[tailleLigne * (nbLigne)-1] = 4*tab[tailleLigne * (nbLigne)-1]+ 2*tab[tailleLigne * (nbLigne)-1 - tailleLigne];
